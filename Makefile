...
index.html: generate-index $(symbole) $(indices) $(ordnersymbole)
	@echo $@ hängt ab von $^
	@echo aber nur $? ist neuer als $@
	./generate-index $(ordnerliste) > index.html

.PHONY: install
install:
	cp -av index.html grabplatte.jpg robots.txt style.css $(destination)/
